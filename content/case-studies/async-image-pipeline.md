...
- Server backstop | The server never trusts the client: magic-byte sniffing (JPEG/PNG/WebP — rejects `INVALID_FILE`, derives `Content-Type` from the bytes, never trusts the extension) and explicit per-endpoint request-body limits that killed the 100 MB-Vehicle `413` class. The same backstop design (ADR-0010) carries EXIF/GPS metadata stripping — already shipping for profile and cover photos — and edge-resized delivery variants from one stored original via the CDN Optimizer.
