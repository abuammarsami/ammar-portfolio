...
- Create idempotency | A retried "start payment" can't double-charge — `dbo.PaymentCreate` dedupes on a per-user `IdempotencyKey` unique index. On top of that, a MediatR idempotency behavior (ADR-0008; required header + SHA-256 body hash) backs a curated 13-endpoint rollout of the highest-risk commands.
